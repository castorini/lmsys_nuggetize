...
def create_and_assign_nuggets():
    ...
    with open(
        "/mnt/users/s8sharif/search_arena/with_retrieval_scrape/results.jsonl", "w"
    ) as results_file:
            ...
            try:
                ...
                for url in urls:
                    ...
                    if len(segment) > 32000:
                        print(
                            f"Trimmed the segment for url {url} and qids {qids} from {len(segment)} to 32k"
                        )
                ...
                with open(
                    f"/mnt/users/s8sharif/search_arena/with_retrieval_scrape/nuggets/requests_with_nuggets_{i}.json",
                    "w",
                ) as f:
                    json.dump(
                        {
                            ...
                            "scored_nuggets": [
                                dataclasses.asdict(sn) for sn in scored_nuggets
                            ],
                        },
                        f,
                    )
            ...
            for qid in tqdm(qids):
                ...
                try:
                    for key in ["a", "b"]:
                        ...
                        metrics[key] = calculate_nugget_scores(
                            request.query.qid, nugget_list
                        )
                    with open(
                        f"/mnt/users/s8sharif/search_arena/with_retrieval_scrape/assignments/assigned_nuggets_{i}.json",
                        "w",
                    ) as f2:
                    ...
                    continue
    with open(
        "/mnt/users/s8sharif/search_arena/with_retrieval_scrape/skips.json", "w"
    ) as skipped_file:
        json.dump(
            {
